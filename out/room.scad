...
// 室內淨 8.82(寬) × 10.48(深) × 1.98(高) m  |  模式=real
...
module furniture(){
  ...
  translate([2.905,-0.645,0]) rotate([0,0,270.00]) color(C_BENCH) bench(7.570,1.910,1.370);
  // #1 工作檯(連排) 7.13x2.39x1.13m [arch] -> bench
  translate([-3.722,-0.455,0]) rotate([0,0,270.00]) color(C_BENCH) bench(7.130,2.390,1.130);
  // #2 工作檯(連排) 5.82x0.95x0.79m [arch] -> bench
  translate([0.565,4.784,0]) rotate([0,0,-2.41]) color(C_BENCH) bench(5.816,0.951,0.790);
  // #3 工作桌 2.11x1.03x0.97m [arch] -> bench
  translate([-0.420,3.395,0]) rotate([0,0,-6.71]) color(C_BENCH) bench(2.115,1.027,0.970);
  // #4 櫃 1.11x1.00x0.86m [arch] -> cabinet
  translate([-2.205,3.755,0]) rotate([0,0,-98.28]) color(C_CAB) cabinet(1.112,1.001,0.860);
  // #5 櫃 0.65x0.23x1.19m [arch] -> cabinet
  translate([-3.271,-4.178,0]) rotate([0,0,-18.69]) color(C_CAB) cabinet(0.650,0.227,1.190);
  // #6 櫃 1.24x0.80x1.02m [arch] -> cabinet
  translate([1.505,3.337,0]) rotate([0,0,-5.00]) color(C_CAB) cabinet(1.245,0.803,1.020);
  // #7 邊檯/櫃 2.14x0.57x1.60m [arch] -> cabinet
  translate([-0.340,-4.766,0]) rotate([0,0,180.00]) color(C_CAB) cabinet(2.140,0.570,1.600);
  // #8 器材(推定) 0.66x0.42x1.15m [discovered+vlm] -> genericbox
  translate([-1.577,-0.121,0]) rotate([0,0,90.00]) color(C_MACHINE) genericbox(0.660,0.420,1.150);
  // #9 空壓機 0.54x0.39x1.34m [discovered+vlm] -> compressor
  translate([-1.412,-2.281,0]) rotate([0,0,90.00]) color(C_MACHINE) compressor(0.540,0.390,1.340);
  // #10 木凳 0.63x0.33x0.81m [discovered+vlm] -> stool
  translate([0.868,-4.306,0]) rotate([0,0,0.00]) color(C_DESK) stool(0.630,0.330,0.810);
  // #11 木凳 0.36x0.30x1.50m [discovered+vlm] -> stool
  translate([1.333,-2.791,0]) rotate([0,0,0.00]) color(C_DESK) stool(0.360,0.300,1.500);
  // #12 器材(推定) 0.39x0.27x1.21m [discovered+vlm] -> genericbox
  translate([-1.502,-0.706,0]) rotate([0,0,0.00]) color(C_MACHINE) genericbox(0.390,0.270,1.210);
  // #13 木凳 0.30x0.27x1.45m [discovered+vlm] -> stool
  translate([1.138,-2.371,0]) rotate([0,0,90.00]) color(C_DESK) stool(0.300,0.270,1.450);
  // #14 鑽床 0.50x0.50x1.55m [vlm] -> drillpress
  translate([-2.250,-2.600,0]) rotate([0,0,0.00]) color(C_MACHINE) drillpress(0.500,0.500,1.550);
  // #15 鑽床 0.50x0.50x1.55m [vlm] -> drillpress
  translate([-2.250,-1.400,0]) rotate([0,0,0.00]) color(C_MACHINE) drillpress(0.500,0.500,1.550);
}
module lights(){
...
module wallitems(){
  color(C_SCREEN) translate([-2.400,-5.071,0.600]) cube([2.000,0.040,1.700]); // 投影布幕[vlm]
  color(C_CAB) translate([-4.937,-4.600,0.900]) cube([0.050,1.700,1.400]); // 藍色公告欄[vlm]
  color(C_AC) translate([3.667,-3.200,1.650]) cube([0.220,0.900,0.300]); // 牆掛冷氣[vlm]
  color(C_AC) translate([0.600,5.194,1.650]) cube([0.900,0.220,0.300]); // 牆掛冷氣[vlm]
}
module ceilingitems(){
  color(C_DUCT) translate([-1.300,-3.600,1.080]) cylinder(h=0.900,r=0.130); // 螺旋通風管[vlm]
}
color(C_WALL) walls();
floorplane();
skirting(); windetail();
